...
export class EWDXReceiver extends EWDX {
	...
	getStaticInformation(): void {
		...
		if (this.model == DeviceModel.EM4 || this.model == DeviceModel.EM2_DANTE) {
	...
	publishVariableValues(): void {
		this.context.setVariableValues({
			...
			...(this.model == DeviceModel.EM4 || this.model == DeviceModel.EM2_DANTE
				? {
			...
			if (json.device.network != undefined) {
				if (json.device.network.dante != undefined) {
					...
					if (json.device.network.dante.interface_mapping != undefined) {
						this.danteInterfaceMapping =
							DantePortMapping[json.device.network.dante.interface_mapping as keyof typeof DantePortMapping]
					}
